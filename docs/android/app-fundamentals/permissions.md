...
`ActivityCompat.ShouldShowRequestPermissionRational`方法用于确定是否应为用户显示基本原理。 此方法将返回`true`如果应显示给定的权限的基本原理。 此屏幕截图显示解释为什么应用程序需要知道设备的位置的应用程序会显示 Snackbar 的示例：
