# [跨平台](index.yml)
## [入门](get-started/index.md)
### [要求](get-started/requirements.md)
### [安装](get-started/installation/index.md)
#### [在 Visual Studio on Windows 中安装 Xamarin](get-started/installation/windows.md)
#### [安装 Visual Studio for Mac](/visualstudio/mac/installation/)
#### [Xamarin 防火墙配置说明](get-started/installation/firewall.md)
#### [卸载 Xamarin](get-started/installation/uninstalling-xamarin.md)
### [移动开发简介](get-started/introduction-to-mobile-development.md)
### [移动 SDLC 简介](get-started/introduction-to-mobile-sdlc.md)
## [应用程序基础知识](app-fundamentals/index.md)
### [辅助功能](app-fundamentals/accessibility.md)
### [生成跨平台应用程序](app-fundamentals/building-cross-platform-applications/index.md)
#### [概述](app-fundamentals/building-cross-platform-applications/part-0-overview.md)
#### [第 1 部分 - 了解 Xamarin 移动平台](app-fundamentals/building-cross-platform-applications/part-1-understanding-the-xamarin-mobile-platform.md)
#### [第 2 部分 - 体系结构](app-fundamentals/building-cross-platform-applications/part-2-architecture.md)
#### [第 3 部分 - 建立 Xamarin 跨平台解决方案](app-fundamentals/building-cross-platform-applications/part-3-setting-up-a-xamarin-cross-platform-solution.md)
#### [第 4 部分 - 处理多个平台](app-fundamentals/building-cross-platform-applications/part-4-platform-divergence-abstraction-divergent-implementation.md)
#### [第 5 部分 - 实际代码共享策略](app-fundamentals/building-cross-platform-applications/part-5-practical-code-sharing-strategies.md)
#### [第 6 部分 - 测试和应用商店审批](app-fundamentals/building-cross-platform-applications/part-6-testing-and-app-store-approvals.md)
#### [案例研究：Tasky](app-fundamentals/building-cross-platform-applications/case-study-tasky.md)
### [代码共享选项](app-fundamentals/code-sharing.md)
### [可移植类库](app-fundamentals/pcl.md)
### [共享项目](app-fundamentals/shared-projects.md)
### [.NET Standard](app-fundamentals/net-standard.md)
### [NuGet 包（自动）](app-fundamentals/nuget-multiplatform-libraries/index.md)
#### [现有库项目](app-fundamentals/nuget-multiplatform-libraries/existing-library.md)
#### [新的多平台库](app-fundamentals/nuget-multiplatform-libraries/single-codebase.md)
#### [新的特定于平台的库](app-fundamentals/nuget-multiplatform-libraries/platform-specific.md)
#### [元数据指南](app-fundamentals/nuget-multiplatform-libraries/metadata.md)
### [NuGet 包（手动）](app-fundamentals/nuget-manual.md)
### [本地化](app-fundamentals/localization.md)
### [传输层安全性 (TLS)](app-fundamentals/transport-layer-security.md)
## [平台功能](platform/index.md)
### [异步支持概述](platform/async.md)
### [C# 6 语言功能](platform/csharp-six.md)
### [F#](platform/fsharp/index.md)
#### [入门](platform/fsharp/overview.md)
#### [F# 示例](platform/fsharp/samples.md)
### [Visual Basic.NET](platform/visual-basic/index.md)
#### [Xamarin iOS 和 Android](platform/visual-basic/native-apps.md)
#### [Xamarin.Forms](platform/visual-basic/xamarin-forms.md)
### [Razor HTML 模板](platform/razor-html-templates/index.md)
## [数据和云服务](data-cloud/index.md)
### [连接的服务](data-cloud/connected-services.md)
### [Azure Active Directory](data-cloud/active-directory/index.md)
#### [入门](data-cloud/active-directory/get-started/index.md)
##### [步骤 1.注册](data-cloud/active-directory/get-started/register.md)
##### [步骤 2.配置](data-cloud/active-directory/get-started/configure.md)
#### [访问 Graph API](data-cloud/active-directory/graph.md)
### [Azure 移动应用](data-cloud/mobile-apps.md)
### [Web 服务简介](data-cloud/web-services/index.md)
#### [演练 - 使用 WCF](data-cloud/web-services/walkthrough-working-with-wcf.md)

## [部署和测试](deploy-test/index.md)
### [自定义链接器配置](deploy-test/linker.md)
### [性能](deploy-test/memory-perf-best-practices.md)
### [使用 NuGet 安装 NUnit 2.6.4](deploy-test/installing-nunit-using-nuget.md)
### [多进程调试](deploy-test/multi-process-debugging.md)
## [Apple 平台](macios/index.md)
### [Apple 帐户管理](macios/apple-account-management.md)
### [Unified API](macios/unified/index.md)
#### [概述](macios/unified/overview.md)
#### [更新现有应用](macios/unified/updating-apps.md)
#### [更新现有 iOS 应用](macios/unified/updating-ios-apps.md)
#### [更新现有 Mac 应用](macios/unified/updating-mac-apps.md)
#### [更新现有 Xamarin.Forms 应用](macios/unified/updating-xamarin-forms-apps.md)
#### [将绑定迁移到 Unified API](macios/unified/update-binding.md)
#### [将代码更新为 Unified API 的提示](macios/unified/updating-tips.md)
### [绑定 Objective-C](macios/binding/index.md)
#### [概述](macios/binding/overview.md)
#### [绑定 Objective-C 库](macios/binding/objective-c-libraries.md)
#### [绑定类型参考](macios/binding/binding-types-reference.md)
#### [目标 Sharpie](macios/binding/objective-sharpie/index.md)
##### [入门](macios/binding/objective-sharpie/get-started.md)
##### [工具和命令](macios/binding/objective-sharpie/tools.md)
##### [功能](macios/binding/objective-sharpie/platform/index.md)
###### [ApiDefinitions 和 StructsAndEnums](macios/binding/objective-sharpie/platform/apidefinitions-structsandenums.md)
###### [绑定本机框架](macios/binding/objective-sharpie/platform/native-frameworks.md)
###### [验证属性](macios/binding/objective-sharpie/platform/verify.md)
##### [示例](macios/binding/objective-sharpie/examples/index.md)
###### [Xcode](macios/binding/objective-sharpie/examples/xcode.md)
###### [CocoaPod](macios/binding/objective-sharpie/examples/cocoapod.md)
###### [高级（手动）](macios/binding/objective-sharpie/examples/advanced.md)
##### [发布历史记录](macios/binding/objective-sharpie/releases.md)
#### [疑难解答](macios/binding/troubleshooting.md)
### [本机引用](macios/native-references.md)
### [本机类型](macios/nativetypes.md)
### [32 位和 64 位平台注意事项](macios/32-and-64/index.md)
#### [将 Xamarin.Mac 统一应用更新为 64 位](macios/32-and-64/mac-64-bit.md)
### [使用跨平台应用中的本机类型](macios/native-types-cross-platform.md)
### [HttpClient 堆栈和 SSL/TLS 实现选择器](macios/http-stack.md)
## [内部机制](internals/index.md)
### [可用程序集](internals/available-assemblies.md)
## [疑难解答](troubleshooting/index.md)
### [常见问题](troubleshooting/questions/index.md)
#### [是否可以通过 Xamarin 使用 Visual Studio 2017 候选发布版本？](troubleshooting/questions/visualstudio-2017-rc.md)
#### [如何查看 PCL 中支持的库？](troubleshooting/questions/pcl-support-libraries.md)
#### [PCL 反射 API](troubleshooting/questions/pcl-reflection.md)
#### [PCL 案例研究：如何解决与 Microsoft TPL 数据流 NuGet 包的 System.Diagnostics.Tracing 相关的问题？](troubleshooting/questions/pcl-case-study.md)
#### [如何更新 NuGet？](troubleshooting/questions/nuget-update.md)
#### [如何降级 NuGet 包？](troubleshooting/questions/nuget-package-downgrade.md)
#### [在更新 Nuget 包后出现缺失包错误](troubleshooting/questions/nuget-packages-missing.md)
#### [统一 Google Play Services 组件和 NuGet](troubleshooting/questions/gps-components-nuget.md)
#### [组件保存在计算机的什么位置？](troubleshooting/questions/component-storage.md)
#### [在哪里可以找到我的版本信息和日志？](troubleshooting/questions/version-logs.md)
#### [应何时以及如何提交 bug 报告？](troubleshooting/questions/howto-file-bug.md)
#### [为什么 Xamarin 不支持 Jenkins？](troubleshooting/questions/xamarin-jenkins.md)
#### [调试器需要哪些项目设置？](troubleshooting/questions/debugger-settings.md)
### [更新对 NuGet 的组件引用](troubleshooting/component-nuget.md)
### [支持选项](troubleshooting/support-options.md)
### [旧许可问题](troubleshooting/legacy-licenses/index.md)
#### [Xamarin 平台团队管理](troubleshooting/legacy-licenses/team-management.md)
#### [如何更改帐户的电子邮件地址？](troubleshooting/legacy-licenses/change-email.md)
#### [如何手动重新同步 Xamarin 许可证？](troubleshooting/legacy-licenses/resync-licenses.md)
#### [一些特定的授权错误](troubleshooting/legacy-licenses/licensing-errors.md)
#### [为什么无法通过 Visual Studio 或 Visual Studio for Mac 登录到 Xamarin？](troubleshooting/legacy-licenses/login-troubleshooting.md)
## [示例](samples/index.yml)
