...
description: 本文档介绍 HealthKit 中的运行状况相关信息提供集中的、 协调，且安全的数据存储的 iOS 8 中引入的框架。 它讨论如何设置的 HealthKit 应用以及如何编写使用 HealthKit 框架的代码。
ms.prod: xamarin
...
> [!IMPORTANT]
> IOS 8 中引入了运行状况工具包。 目前，运行状况工具包不是可在 iOS 模拟器上和调试需要连接到物理 iOS 设备。
...
> [!IMPORTANT]
> 要清楚地了解此函数的自变量：_成功_和_错误_参数不会指示用户是否已授予权限以访问运行状况工具包数据 ！ 它们仅指示用户已被授予机会允许对数据的访问。
...
> [!IMPORTANT]
> 与设置相关的错误将在此时呈现。 若要解决错误，请查看创建和运行状况工具包应用上述设置。 组件包括： 
>
